...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          let raffle, vrfCoordinatorV2Mock, raffleEntranceFee, player, interval, accounts
          ...
          describe("constructor", function () {
              it("sets raffle state correctly", async function () {
              ...
              it("sets number of words correctly", async function () {
                  const numWords = await raffle.getNumWords()
                  assert.equal(numWords.toString(), "1")
              })
              it("sets request confirmations correctly", async function () {
          ...
          describe("performUpkeep function", function () {
              it("can only ran if checkUpkeep is true", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
